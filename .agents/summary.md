...
>
| **Last updated: 2026-08-31 (Polish-3 complete — DEBT D28 closed. `--ls-cool` promoted to three-tier `--color-cool{,-soft,-dim}` in `packages/ui/src/tokens.css` `@theme` + light overrides; the 2 inline defs in `apps/web/components/home/home.css` removed; the 2 use sites in `.ls-tag-concept` renamed to `var(--color-cool)`. `docs/DEBT.md` D28 row updated. Branch `polish/d20-cool-tokens` cut off develop directly (NOT main) — 5-min PR; off-main merge-conflict cost would exceed the work itself. All 3 gates green: typecheck, next build 236/236, verify:prerender 196/196+18/18. Polish-1 + Polish-2 also merged (PRs #92 + #93) — develop at `9822de5`. D13 still blocks `verify:links` (44/33). `origin/main` 11 commits behind `origin/develop`. **Next:** Polish-4 (D21 Pagefind + ⌘K); Polish-5 (View Transitions API on lessons); develop→main release PR is reserved for user action.**)
---
