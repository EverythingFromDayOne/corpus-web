...
>
| **Last updated: 2026-08-31 (Polish-7 in flight — `polish/d22-seo-residue` cut off `develop`. DEBT D22 partially closed: `/sitemap.xml` (App Router route handler emitting 219 URLs: 3 listing × 1 locale + 2 course details + 18 lessons + 196 adapting articles; reuses cached `getCatalogView()`) and `/robots.txt` (`User-agent: *`, `Allow: /`, `Disallow: /api/`, `Sitemap:` pointer). `.gitignore` updated with `apps/web/public/pagefind/` build output (brings forward the entry from PR #97 which has already merged). OG image generation to `cdn.nxhhuy.tech` remains open — the CDN sub-domain requires DNS + Vercel project routing config (cross-session / deployment-config change, beyond autonomous principal-engineer scope). All 3 gates green: typecheck 5/5, next build 236/236 + 2 new static routes (`/sitemap.xml`, `/robots.txt` show as `○` static in build table), verify:prerender 196/196+18/18, verify:frontmatter 196 articles adapt. Brand-string + personal-content guards: 0 hits on shipped strings. Predecessor PRs: #96 Polish-5 batch-5 merged 2026-08-31, #97 Polish-6 merged 2026-08-31. Polish-8 PR #99 (D25 `/en/license` + footer) still MERGEABLE on develop behind PR #98. **Next:** Polish-9 candidates per Phase 1 residue (D20 Shiki ⚪, D29 category-filter wiring, D32 related-articles section with 289 edges already in catalog); OG image generator (D22 residue — needs DNS + Vercel routing green-light in its own session); develop→main release PR remains reserved for user action.**)
---
