...
>
| **Last updated: 2026-08-31 (Polish-5 complete — `polish/d20-batch-5-blog-typography` cut off `develop`. 5 small additive items: hero bloom + gradient text on `/en` (home §2); `.blog-content` typography block (blog §15 High, 16px/1.7 lh, 768px reading column, scoped under `[data-blog]`); post-header template for blog posts (badge + H1 + 4-piece meta row); second use site of the section-divider primitive on `/en/blog`; new `[data-blog]` wrapper layout + 15 `--blog-*` scoped tokens (dark + light variants) in `packages/ui/src/tokens.css`. All 3 gates green: typecheck, next build 236/236, verify:prerender 196/196+18/18. New component `apps/web/components/article/post-header.tsx` (~48 lines, vendor-neutral). New CSS `apps/web/components/article/blog-content.css` (171 lines). New layout `apps/web/app/[locale]/blog/layout.tsx`. App Router `<html>`-level `data-blog` deferred to a wrapping div per spec §14 caveat. D13 still blocks `verify:links` (44/33). **Next:** Polish-6 (D21 Pagefind + ⌘K); Polish-7 (D22 SEO residue: sitemap + OG images + robots.txt); develop→main release PR is reserved for user action. Polish-3 (D28) + Polish-4 (audience cards PR #93) + Polish-5 (View Transitions PR #95) merged earlier today.**)
---
