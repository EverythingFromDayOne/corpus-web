...
>
| **Last updated: 2026-08-31 (Polish-10 in flight — `polish/d30-timeline-visual` cut off `develop`. DEBT D30 partial close (timeline half): `apps/web/components/courses/course-card.tsx` `<CurriculumList>` now renders the existing `course.items[]` as a vertical learning-path timeline with left-rail filled dots (first + last items only), `border-l` connector segments between non-final steps, zero-padded ordinals in `tabular-nums`, and the per-step `note` rendered as a `border-l-2 italic` callout that visually distinguishes the rationale from the lesson title. `<ol>` carries `aria-label="Learning-path timeline"` (`courses.curriculumTimelineLabel`) for assistive tech. `apps/web/messages/en.json` adds the new i18n key under the existing `courses.*` namespace. **D30's FAQ accordion half remains open** — schema has no `Path.faqs` field; needs coordinated corpus-side authoring. 45 +/− 12 in the component, +1 i18n key. All 5 gates green: typecheck 5/5, lint 0 problems, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196. HTML spot-check on `/en/courses/react-foundations`: 6 `<li class="timeline-step">`, 2 filled dots, 5 connectors, 6 note callouts — matches the data shape exactly. Brand-string + personal-content guards: 0 hits. Predecessor PRs: #96 Polish-5 batch-5, #97 Polish-6, #98 Polish-7, #99 Polish-8, #100 Polish-9 hotfix, #101 Polish-9 D29 partial. **Next:** Polish-11 candidates per Phase 1 residue — D20 Shiki code blocks (only remaining ⚪ Phase-1 row; new dep `shiki` ~50MB grammars — needs user green-light per session protocol), D30 FAQ accordion (schema field `Path.faqs` — needs corpus-side authoring), D32 related-articles polish (cosmetic guard against D13 unresolved refs leaking as `<a>`); OG image generator (D22 residue — needs DNS + Vercel routing green-light in its own session); develop→main release PR remains reserved for user action.**)
---
