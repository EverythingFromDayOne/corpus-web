...
>
| **Last updated: 2026-09-04 (develop → main promotion: PR #161 merged at `16fecf7`, ADR-0003 accepted at `957eeaf`, v0.1.0 tagged at `c7e2e23`). The chain: D38 close (`4a050c8`) → D13 close + D46 open (`4851dc3`) → nav-progress fix (`15d1213`, PR #159) → favicon + home title (`7e4846f`, PR #160) → conflict resolution on develop (`9f5b99a`) → develop → main merge with `--no-ff` per ADR-0003 Option A (`16fecf7`). `required_linear_history` on main discovered as the mechanism that forced squash-merge at the GitHub API layer for the previous three promotions; disabled via `gh api PUT` and stays disabled per user direction. Promotion protocol documented in ADR-0003 (`docs/adr/0003-promotion-strategy.md`): verify D46-only red → conflict-resolve toward develop → open PR with per-file table → `gh pr merge --admin -m` (NOT `--squash`) → confirm Vercel deploys main → run the four curl checks against the production alias → tag the merge commit. Verified on `nxhhuy.tech` post-merge: `<title>corpus.web — verified reference for web engineering</title>`, `<h1>Every claim resolves.</h1>`, `/favicon.ico` 200 (2747 B, MS Windows icon resource, 3 sizes 16/32/48), `/en/courses/react-render-cycle/lessons/jsx-and-rendering` (lesson with quiz) 200 (502 KB).** 
