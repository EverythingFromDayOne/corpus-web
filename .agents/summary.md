...
>
| **Last updated: 2026-08-31 (Polish-blog-card-hover — `polish/blog-card-hover` off `develop` @ `bd33ebd`, PR #109. **Article-card hover lift on `/en/blog`**: added `group-hover:-translate-y-0.5` + `group-hover:shadow-[0_4px_12px_color-mix(in_srgb,var(--color-ink)_30%,transparent)]` to the existing `<a>` card className; `transition-colors` → `transition-[transform,box-shadow,border-color]` so only the three properties that change on hover animate. Cards already had a left accent bar that draws in (`group-hover:scale-y-100`) and a border-colour transition; what was missing was the lift — they read as "this row" but not as "this row I want to click." Tailwind v4 emits the new rules inside `@media (hover: hover){...}` so touch devices get only the existing colour/border feedback (correct — touch already has its own press state). Invented decision: `translate-y-0.5` instead of `scale-105` because the cards have no thumbnail; `scale-105` would just enlarge the text slightly, not deliver the lift the design needs. 1 file +1/-1, all 5 gates green. Verified the rendered className in served HTML and both rules in served CSS bundle (`/_next/static/chunks/33zmoq-xlm6uy.css`). PR #109.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
