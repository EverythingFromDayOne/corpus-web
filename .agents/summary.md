...
>
| **Last updated: 2026-08-30 (Hermes-Coding handover kit authored on `develop` at `b58749c`. Two new files under `prompts/`: `HANDOFF-corpus-web.md` (~700 lines, base kit) + `HANDOFF-session-protocol.md` (~120 lines, per-session supplement). The kit is loaded only by sub-agent task prompts; not user-visible. Output shape is a fixed template per the user's request: working-process detail in tool calls + SESSION-LOG, not in the response. PR #91 (pill theme toggle) MERGED at `b58749c` earlier; `develop.nxhhuy.tech` shows SVG sun + moon + sliding thumb. PR #90 (card hover + film-grain + share buttons) MERGED at `29182d4`. 0 open PRs. **Next:** run a sub-agent through a real task to validate the kit; queued polish (skeleton placeholders ~2h / audience cards ~2h / three-tier accent tokens ~2h breaking); doc-state cleanup (~30 min); D13 follow-up (~1h).)**
