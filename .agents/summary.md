...
>
| **Last updated: 2026-08-31 (Polish-search-fixes in flight — `polish/search-fixes` cut off `develop`. Five regressions reported on `develop.nxhhuy.tech` after the Polish-6/7/8/9/10/11 batch landed; four fixed, fifth's diagnostic improved. **Issue 1 — moon icon tight to left**: `apps/web/components/chrome/theme-toggle.tsx` thumb translates to `translate-x-8` (was `translate-x-9`), icon spans get `shrink-0` + `text-[0.95rem] leading-none` so moon/sun sit centred in their halves instead of being clipped against the thumb. **Issue 2 — placeholder clipped to "Search 196 a."**: `apps/web/components/chrome/search-trigger.tsx` drops the leading `<span class="meta">SEARCH</span>` label and replaces with an inline `<svg>` magnifier; `apps/web/app/globals.css` `.srch` widens from 15rem max to a fixed 16rem so `"Search 196 articles…"` no longer ellipses. **Issue 3 — dialog top-left instead of centred**: `.srch-dialog` now `position: fixed; inset: 0; margin: auto; height: max-content; max-height: 70vh;` — native `<dialog>` doesn't centre unless both inset and margin:auto are applied. **Issue 4 — search panel icons collapsed**: `.srch-dialog-input` gap widens to 0.75rem, padding becomes `0.25rem 0.25rem 0.75rem`, and `.srch-dialog-input > svg` gets explicit `flex: none; width: 16px; height: 16px; color: var(--color-muted);` so the magnifier doesn't squish against the `<input>`. **Issue 5 — "Search failed"**: `apps/web/components/chrome/search-dialog.tsx` `status` switches from a string union to a discriminated union `{ kind: 'idle' \| 'loading' \| 'ready' \| 'empty' } \| { kind: 'error'; message: string }`; both error paths extract the underlying `Error.message` and surface it in monospaced grey text below the "Search failed. Try again." line. Pure visual + diagnostic; Pagefind search itself is unchanged when the bundle loads successfully. Dead-code removal: `.srch input { … }` rule deleted (trigger never had an `<input>` child — leftover from the disabled placeholder, inert under `.srch-trigger`). 4 files changed, +79 / −35. All 5 gates green: typecheck 5/5, lint 0 problems, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196. HTML spot-check on `/en/blog.html`: search trigger now `<svg>` + full `Search 196 articles…` + `⌘K`, no ellipsis. Brand-string + personal-content guards: 0 hits. **Next:** same residue (D20 Shiki, D30 FAQ, D22 OG image, develop→main release) — all need user green-light. **If search "Search failed" reappears in production, the new error detail line will surface the underlying `Error.message` (worker init failure / wasm MIME / fetch 404 etc.) so the diagnosis is one paste away.** |
---
