...
>
| **Last updated: 2026-08-31 (Polish-1 — lesson-route skeleton placeholders shipped on `polish/d20-skeleton @ cb82fcc`. NEW `apps/web/components/lesson-skeleton.tsx` (chrome + 3 paragraph + 2 callout + 1 table + 1 code-block, all `bg-muted motion-safe:animate-pulse rounded`, outer `aria-hidden`) + `<Suspense fallback={<LessonSkeleton />}>` wrap in `apps/web/app/[locale]/courses/[course]/lessons/[slug]/page.tsx`. Mounted under `<Suspense>`, not as first-paint — Cache Components streams the static HTML immediately, skeleton is the fallback for any future streaming subtree. Sub-agent (Hermes-Coding profile, `--run-budget 1500`) returned clean in ~9 min; principal engineer re-ran all 4 gates (`pnpm typecheck` clean, `next build` 236/236, `pnpm verify:prerender` 196/196 blog + 18/18 lessons, brand + personal guards 0). `pnpm verify:links` still fails on pre-existing D13 (44 unresolved refs across 33 targets in nextjs + nestjs); not introduced by this PR. Path B worked end-to-end — spec at `/tmp/skeleton-task.txt` (330 lines, kit-shaped, via `--query-file`), sub-agent session `20260831_011919_bc8558`, branch on origin. **Next:** user review + open PR to `develop` (squash); then Polish-2 candidates (audience cards ~2h, three-tier accent tokens ~2h breaking, D21 Pagefind); `~/.hermes/profiles/coding/PROJECTS.md` and the worked-example cache file need 1-line patches for stale workflow-flag guidance; develop→main release PR is pending (origin/main 9 commits behind origin/develop).)**
