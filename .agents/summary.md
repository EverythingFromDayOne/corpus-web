...
>
| **Last updated: 2026-08-31 (Polish-blog-card-kind-badge — `polish/blog-card-kind-badge` off `develop` @ `606474d`, PR #112. **Kind badge overlay on `/en/blog` article cards** (design-spec blog §3). Cards previously rendered title + description + corpus · reading-time but had no visual indication of **what kind** of article each one was (concept vs recipe), even though every `ArticleListItem` already carries `kind: 'concept' | 'recipe'` in the catalog. Closed the gap by adding a `<span class="tag-soon ls-tag-concept">Concept</span>` / `<span class="tag-soon ls-tag-recipe">Recipe</span>` badge to each card's meta row, reusing the existing classes from `apps/web/components/home/home.css` (already used for home-page entry-point chips, with the spec's exact colors — cool for concept, signal for recipe). Meta row converted from inline `·`-separated text to `flex flex-wrap items-center gap-2` so the badge fits cleanly with the existing corpus + reading-time. Each badge has `aria-label="Kind: Concept"` / `"Kind: Recipe"` for assistive tech, using the existing `article.kind` i18n key as the prefix. **Honest re-scoping note**: my earlier "design-spec backlog" surface (in the Polish-items-left turn) listed film-grain, hero bloom, view-transitions, and share buttons as 30-min candidates — all four turned out to be already implemented. Re-grep'd for `Gap:` annotations and found this was the only real small-additive gap. 1 file +18/-13, all 5 gates green. Verified `/en/blog` HTTP 200 in 22ms; 196 `tag-soon ls-tag-*` badges total (134 `concept` + 62 `recipe`) in the rendered HTML — matches the catalog split 1:1. **Session cadence cap: hit** — six polish batches chained in this run (PRs #107 + #108 + #109 + #110 + #111 + #112). PR #112.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
