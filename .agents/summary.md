...
>
| **Last updated: 2026-08-31 (Polish-search-spotlight-ux in flight — `polish/search-spotlight-ux` cut off `develop` @ `72239fe`. **4 search-dialog regressions fixed + Spotlight-style UX**: (1) modal visible on first load → CSS `.srch-dialog[open]` scoped layout + explicit `.srch-dialog:not([open]) { display: none }` defensive rule (the old layout on `.srch-dialog` overrode the UA `dialog:not([open]) { display: none }` and made the dialog paint visibly without `showModal()`); (2) click-outside-doesn't-close → capture-phase click listener on the dialog checks `e.target === dialog` and calls `dialog.close()`; (3) "Searching…" stuck forever in dev → match rejected-import message against dev-mode signals (`/Failed to fetch|404|MIME type|Loading module|Loading chunk|NetworkError/i`) and append an actionable hint pointing the user at `pnpm start` (Pagefind only ships after `pnpm build`'s `postbuild` hook); (4) delete word-by-word leaves stale results → monotonic `requestIdRef` stamps every fired query, debounced handler captures the id and bails the response if a newer keystroke has already superseded it. UX upgrades: inline clear-X button replacing the `⌘K` chip when query is non-empty (Spotlight convention); fixed-height top-anchored panel with `flex: 1 1 auto; min-height: 0` on the inner list so it scrolls inside the panel instead of re-growing it (root cause of "panel tears as results arrive"); `scrollIntoView({ block: 'nearest' })` on active row change; modular result row (bold title + small muted breadcrumb + two-line-clamped excerpt) derived from the URL via `titleFromUrl()` + `breadcrumbFromUrl()`; idle-state hint in the empty list. 2 files +192/-78, 1 i18n file +2 keys, all 5 gates green. User spot-check on `develop.nxhhuy.tech` is the functional gate. PR #108.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
