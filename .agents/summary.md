...
>
| **Last updated: 2026-08-31 (Polish-9 ready — `polish/d29-blog-kind-filter` cut off `develop`. DEBT D29 partial close (blog half): `apps/web/components/blog/article-index.tsx` now exposes TWO filter axes — corpus (All / Next.js / Angular / React / NestJS) and kind (All / Concept / Recipe) — composed in a single useMemo and rendered as two `role="group"` chip rows above the article grid. `apps/web/messages/en.json` splits `blog.filterLabel` ("Filter by corpus") into `blog.filterCorpusLabel` + `blog.filterKindLabel`, and rewrites `blog.empty` to reflect the combined-filter state ("No articles match these filters."). Single-component change, +47/−23 in `article-index.tsx`, +3 keys in `en.json`. `/en/courses` half left genuinely inert-by-design (only 2 courses ship today; an axis of 2 chips for 2 items is dead UI). All 3 gates green: typecheck 5/5, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196, lint 0 problems. Brand-string + personal-content guards: 0 hits. Phase 1 status: D25 ✅, D21 ✅, D22 🟢 (sitemap + robots.txt shipped; OG images deferred), D28 ✅, D29 🟢 (partial), D18 ✅ — only D20 ⚪ Shiki code blocks remain in the Phase-1 ⚪ pile. PR #100 (Polish-9 ESLint Pagefind-output exclude) MERGED. Predecessor PRs: #96 Polish-5 batch-5, #97 Polish-6, #98 Polish-7, #99 Polish-8, #100 Polish-9 hotfix. **Next:** Polish-10 candidates per Phase 1 residue — D20 Shiki code blocks (only remaining ⚪ Phase-1 row; new dep `shiki` ~50MB grammars — needs user green-light per session protocol), D30 course overview (FAQ + learning-path timeline — FAQ needs schema work), D32 related-articles section (already partially rendered; 289 intra-corpus edges, 94 articles have zero `related`, 4 of 5 links rendered as `<span>` for 1 sample due to D13 unresolved refs); OG image generator (D22 residue — needs DNS + Vercel routing green-light in its own session); develop→main release PR remains reserved for user action.**)
---
