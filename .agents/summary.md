...
>
| **Last updated: 2026-09-03 (Polish-d22-static-og — `polish/d22-static-og` off `develop @ 2e52548`, PR #152, squash `9bbd2f1`. **D22 close**. 9 files +267/-5. Single static `/opengraph-image` route at `apps/web/app/opengraph-image.tsx` via `next/og`'s `ImageResponse` (Satori, bundled with Next 16.3.1, zero new npm dep), 1200×630 PNG, mirrors the home-hero palette discipline (`--color-ink` ground + warm `--marketing-accent-bloom` upper-right + cool `--color-cool` lower-left). Fonts bundled at `apps/web/public/og-fonts/` (Archivo-Bold 111KB + IBMPlexMono-Regular 133KB). `og:image` + `twitter:card` metadata wired into all 5 surface `generateMetadata` functions (home / blog index / blog article / course detail / lesson) via shared `OG_IMAGE_*` constants in `apps/web/lib/site.ts`. Per user decision in session 161: static shared fallback over dynamic per-article — site has no per-article art and social sharing volume is near zero. **Verification**: typecheck 5/5, lint 5/5, test 39/39, `pnpm --filter @corpus/web build` clean, `pnpm verify:prerender` 196/196 + 18/18, `pnpm verify:frontmatter` 196/196, `pnpm agents:check` ✓, `hermes verify --json` ok=true 9/9 phases PASS readiness HTTP 200 in 0.104s. Live probe `/opengraph-image` returns HTTP 200, `image/png`, 1200×630, 85.9KB; live probe `/en` and `/en/blog/nextjs/cache-components-model` confirm `<meta property="og:image">` + `<meta name="twitter:card">` with width/height/alt correctly emitted. **Build-time memory headroom** required `NODE_OPTIONS='--max-old-space-size=4096'` to avoid a worker-pool SIGKILL on the first retry — same 241-page output, no code change. D22 row in `docs/DEBT.md` closes with this PR. **D20 Shiki** (build-time variant) is the next in-scope polish item from the session 161 user decision — same `polish/*` branch pattern off this `develop @ 9bbd2f1` head. **D21** Pagefind on Preview still blocked on Vercel Auth dashboard bypass for `/pagefind/*` + `/opengraph-image` + article route URLs (user-action, no code). **D38 informational `verify-links` fail** (44 unresolved `related` refs in nextjs + nestjs submodules) is pre-existing and unchanged.
...
## Polish residue (Session 161, 2026-09-03)

**Session 161 D22 close.** `polish/d22-static-og` off `develop @ 2e52548`, PR #152 squash `9bbd2f1` — `/opengraph-image` route + `og:image`/`twitter:card` metadata on all 5 surfaces. Single static design per user decision ("site has no per-article art and social sharing volume is near zero"). D22 row in `docs/DEBT.md` closed.

**Remaining polish residue:**
- **D20 Shiki** — **decided A (build-time variant, devDep)**. User's session 161 rationale: "On a reference corpus, code blocks aren't illustrative, they're the payload." Build-time highlights + inline styles = no runtime bundle cost. Dual-theme keyed to existing tokens (not stock GitHub Dark). Queued for next sub-action this session.
