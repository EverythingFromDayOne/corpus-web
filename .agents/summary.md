...
>
| **Last updated: 2026-08-31 (Polish-web-start-script — `polish/web-start-script` off `develop` @ `74b454c`, PR #110. **Developer-ergonomics fix**: `apps/web/package.json` was missing the standard `start` script that exists in every other Next.js project. `pnpm --filter @corpus/web start` errored with `ERR_PNPM_RECURSIVE_RUN_NO_SCRIPT`, forcing every prod-serve probe (notably PR #108's mobile-follow-up verification) to fall back to `cd apps/web && npx --no-install next start --port 3000`. Added `"start": "next start --port 3000"` between `build` and `postbuild` to complete the standard Next.js script trio (`dev` / `build` / `start`). The `--port 3000` flag matches the `dev` script's explicit port for predictable contract. No new deps — uses `next` which is already a dependency. 1 file +1 line, all 5 gates green. Verified `pnpm --filter @corpus/web start` boots Next.js 16.3.1, `GET /en` HTTP 200 in 34ms, `/pagefind/pagefind.js` HTTP 200. PR #110.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
