...
>
| **Last updated: 2026-08-31 (Polish-7 in flight — `polish/d22-seo-residue` cut off `develop` (`efe88e8`). DEBT D22 partially closed: new `apps/web/app/sitemap.xml/route.ts` (App Router route handler emitting 219 URLs: 3 listing surfaces × 1 locale + 2 course detail pages + 18 lessons + 196 adapting articles) and `apps/web/app/robots.txt/route.ts` (`User-agent: *`, `Allow: /`, `Disallow: /api/`, `Sitemap:` pointer). Both reuse `getCatalogView()` so they cost O(1) at build time and ship as static text from the edge. `.gitignore` updated with `apps/web/public/pagefind/` (mirrors `polish/d21-pagefind`'s same entry). OG image generation to `cdn.nxhhuy.tech` remains open — the CDN sub-domain is a deployment/DNS-config change beyond autonomous principal-engineer scope (recorded for a session where the user explicitly green-lights the DNS + Vercel project routing setup). All 3 gates green: typecheck 5/5, next build 236/236 + 2 new static routes (`/sitemap.xml`, `/robots.txt` show as `○` static in build table), verify:prerender 196/196+18/18, verify:frontmatter 196 articles adapt. Brand-string + personal-content guards: 0 hits on shipped strings. Polish-6 (PR #97, D21 Pagefind) and Polish-5 batch-5 (PR #96) are both still MERGEABLE on develop; union-merge will resolve append-only docs conflicts when squashed. **Next:** Polish-8 (D25 `/en/license` page); OG image generator (cross-session, needs DNS + Vercel routing setup — separate session); develop→main release PR remains reserved for user action.**)
---
