...
>
| **Last updated: 2026-08-31 (Polish-search-spotlight-ux topbar follow-up — `polish/search-spotlight-ux` off `develop` @ `72239fe`, PR #108. **Topbar search-input-covered bug fixed**: a red-circle annotation on a mobile screenshot showed the search trigger being clipped invisibly against the viewport's right edge because the topbar layout `[hamburger] [logo] [Home Courses Articles] [SearchTrigger 16rem] [ThemeToggle]` was ~640px of content competing for a 390px iPhone viewport. The trigger had `width: 16rem` and no `min-width: 0`, so the flex child refused to shrink and `.topbar-wrap`'s `overflow: hidden` clipped it. Fix: added `min-width: 0` to `.srch` so the flex child CAN shrink on intermediate widths (tablets); new `@media (max-width: 640px)` rule collapses `.srch-trigger` to a 34×34 icon-only button matching the theme toggle's geometry by hiding `.srch-trigger-input` and `.srch-kbd` and zeroing padding. The full input already lives inside the dialog (Spotlight-style per PR #108). iOS Safari uses the same collapse pattern. Verified in served bundle `.next/static/chunks/30s__szcvb5cx.css`: `@media (max-width:640px){.srch-trigger{justify-content:center;width:34px;height:34px;padding:0}.srch-trigger-input,.srch-trigger .srch-kbd{display:none}}`. 1 file +26/-2, all 5 gates green. PR #108 updated.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
