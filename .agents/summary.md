...
>
| **Last updated: 2026-08-30 (PR #91 in flight on `polish/d20-batch-3`: pill theme toggle shipped + fix commit landed. **First commit** = pill with sliding thumb + Unicode-glyph sun/moon (`☀` `☾`) + `bg-signal` thumb. **Visual smoke caught** that Archivo + IBM Plex Mono don't include U+2600/U+263E — browser fell back to OS default and rendered the sun as a dot, the moon as a snowflake/asterisk glyph. **Fix commit** = inline SVG icons + `bg-muted` thumb to balance visual weight. Total: 2 feature commits + 1 docs wrap + 1 merge resolution = 4 commits on `polish/d20-batch-3`. **Lesson:** "glyphs are not icons" — when font fallback is in play, a glyph's shape is unknown until proven. PR #91 status: MERGEABLE / UNSTABLE (CI rerunning on merge commit). PR #90 (card hover + film-grain + share buttons) already MERGED at `29182d4` on `develop` and `develop.nxhhuy.tech` is bound to that branch via Vercel Preview + DNS. User leaning on me; this was the highest per-page-visibility polish item. **Next:** visual smoke at `develop.nxhhuy.tech` (user behind Vercel Auth), then squash-merge. **Next NEXT session:** Hermes-Coding handover-kit authoring (meta-work, separate context).)**
