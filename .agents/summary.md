...
>
| **Last updated: 2026-08-31 (Polish-search-esm-import in flight — `polish/search-esm-import` cut off `develop` @ 8426adc. **Search fixed for real**: Pagefind 1.x's `/pagefind/pagefind.js` is a native ES module (ends with `export{createInstance,…,search}`). The prior code injected it via classic `<script src>` and polled `window.pagefind` for 10s — the `export` keyword was a SyntaxError, so the bundle threw on parse and never registered anything. Three sessions' worth of "Search failed on develop.nxhhuy.tech" traced back to this. Switched `ensurePagefind` to a single dynamic `import('/pagefind/pagefind.js')` + `init()` + `search()`; replaced non-existent `pf.getFragment(r)` with `r.data()` per Pagefind's canonical API. 1 file changed, -70/+44. Verified end-to-end via Chrome DevTools Protocol on both dev and prod builds: typing "angular" in the dialog returns 8 real ranked Angular articles (module-federation, builders, routing, etc.) with `<mark>angular</mark>` excerpts. All 5 gates green: typecheck 5/5, lint 0 problems, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196, vitest 38/38 pass / 0 fail. **Vercel Auth-SSO hypothesis refuted**: Pagefind now works on localhost without any Vercel config change. The 302 → vercel.com/sso-api path on develop.nxhhuy.tech was a red herring; the underlying code path was broken on every environment from day one, and only the redirect happened to mask it differently. PR #107.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be solved by code or `vercel.json`. **Next:** once merged, verify on develop.nxhhuy.tech that (a) search dialog shows "Loading search index…" within 1 frame of typing, (b) progress bar appears on every internal link click + browser back/forward. **Vercel Bypass config action item**: user needs to log into Vercel dashboard → corpus-web → Settings → Deployment Protection → add `/pagefind/*` to the path-based bypass list, then re-test search. Until then, "Search failed" will persist on preview deployments. |
---
