...
>
| **Last updated: 2026-09-05 (D45 close — preventive emit-then-static-import for article bodies, PR #162 opened against develop on `fix/d45-article-source-bodies @ 2c16d66`). Same shape as D43's `fix/d43-answer-keys` (deployed verified via PR #161 / v0.1.0): `scripts/build-article-bodies.mjs` reads `catalog.json`, walks every adapting article from the four content submodules, strips YAML frontmatter, emits `apps/web/lib/data/article-bodies.ts` (3933 KB for 196 articles, gitignored) as a static TS module. `apps/web/lib/article-source.ts` rewritten to consume the static module via a Set-guarded lookup — no `readFileSync` at request time, no ENOENT risk if `readArticleMarkdown` is ever wired into a Server Action. 9/9 hermes verify phases PASS cold (build 73.1s / readiness HTTP 200 in 11.499s); live curl `/en/blog/nextjs/cache-components-model` 200 (839 KB body, h1 + cacheLife mentions confirm static-import rendered the actual markdown) and `/en/courses/react-render-cycle/lessons/jsx-and-rendering` 200 (580 KB). Deployed verification for D45 is structurally weaker than for D43 (preventive fix; no prod-surface bug exists today because no Server Action calls the function — only the prerender pass does, in a build container where fs is readable). 4 MB generated module gitignored to keep repo size sane across content submodule bumps; recovery: `pnpm build:catalog && node scripts/build-article-bodies.mjs`. Previous update: 2026-09-04 (develop → main promotion chain: PR #161 merged at `16fecf7`, ADR-0003 accepted at `957eeaf`, v0.1.0 tagged at `c7e2e23`).** |
...
- **D43 (serverless-fs runtime read pattern)** — closed on 2026-09-04 via `fix/d43-answer-keys` PR (deployed verified on `nxhhuy.tech` via PR #161 / v0.1.0). `quiz-actions.ts` + `dragdrop-actions.ts` rewritten to static-import `apps/web/lib/data/answer-keys.ts` (32 KB build artifact emitted by `scripts/build-answer-keys.mjs` at prebuild time). `catalog.json` and `curation/overrides/*.yaml` no longer read from disk in the action path.
- **D45 (preventive close)** — `fix/d45-article-source-bodies` PR #162 opened against develop on 2026-09-05. Same D43 emit-then-static-import pattern, applied to article bodies. `scripts/build-article-bodies.mjs` (NEW, ~165 lines) reads `catalog.json`, walks every adapting article from the four content submodules, strips frontmatter, emits `apps/web/lib/data/article-bodies.ts` (3933 KB for 196 articles, gitignored) as a static TS module. `apps/web/lib/article-source.ts` rewritten — drops `readFileSync`, consumes the static module via a Set-guarded lookup. **Preventive, not active**: no Server Action currently calls `readArticleMarkdown`; the function is only invoked from the prerender pass, which runs in a container where `content/<repo>/` submodule gitlinks are readable. The fix closes the latent D43-shaped bug for the article body read before any future Server-Action wiring can fire it. 9/9 hermes verify phases PASS cold (readiness 200 in 11.499s); live curl `/en/blog/nextjs/cache-components-model` 200 (839 KB) and `/en/courses/react-render-cycle/lessons/jsx-and-rendering` 200 (580 KB) confirm the static-import renders the actual markdown body. **Deployed verification is structurally weaker than for D43**: no prod-surface bug exists today to reproduce. D45 row flipped to closed in DEBT.md (preventive fix).
- **D46 (19 `related` refs in `content/nestjs/*/...md` point at recipe slugs that exist in no manifest)** — opened 2026-09-04. Replaces D13 as the only FAIL count for `verify-links` after the roadmap-classification reclassification (44 → 19). All 19 are `nestjs` recipes (`recipes/di-and-modules/nest-cant-resolve-dependencies` × 4, `recipes/validation/nested-dto-not-validated` × 2, plus 13 × 1). Closure paths: (a) write the recipe and re-tag the submodule, (b) drop the forward ref from the inbound article's `related:` block, or (c) add a per-recipe-slug manifest to `nestjs/roadmap.md §5` mirroring `nextjs/roadmap.md §4`'s "Planned recipes" subsection. Recommend (c) for symmetry with the nextjs roadmap. **This is a real defect the gate should catch**; the user endorsed this in the previous turn. **Corpus-side fix requires `content/nestjs` submodule worktree — STOP-AND-ASK per AGENTS.md "Touching submodules is forbidden without explicit ask."**
