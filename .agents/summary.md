...
>
| **Last updated: 2026-09-03 (Polish-d20-shiki-buildtime — `polish/d20-shiki-buildtime` off `develop @ be9d25e`, PR #153, squash `46b77e6`. **D20 close**. 7 files +322/-8. Build-time syntax highlighting via `rehype-pretty-code@^0.14.5` + `shiki@^4.4.3` (both in `apps/web/devDependencies`, zero client bundle impact). Two custom Shiki themes at `apps/web/lib/shiki-theme-{dark,light}.json` hand-map every token scope to the existing `@theme` tokens in `packages/ui/src/tokens.css` — NOT stock GitHub Dark / Light, per the user directive in session 161. Dual-theme via `--shiki-dark` / `--shiki-light` CSS custom properties Shiki emits inline on every token span. `onVisitLine` callback + CSS `counter-reset` / `counter-increment` produces the line-number gutter with zero JS. `packages/mdx-components/src/code-block.tsx` adds `isShikiTree` detection to render Shiki token spans verbatim instead of discarding them via the old `\n` split. **Verification**: typecheck 5/5, lint 5/5, test 39/39, `pnpm --filter @corpus/web build` clean, `pnpm verify:prerender` 196/196 + 18/18, `pnpm verify:frontmatter` 196/196, `pnpm agents:check` ✓, `hermes verify --json` ok=true 9/9 phases PASS readiness HTTP 200 in 1.565s. Live probe `/en/blog/nextjs/cache-components-model` returns 25 Shiki figures with 2702 `--shiki-dark` CSS variable tokens + 368 `class="line"` markers. Sample token spans: cyan keywords `#6AA9D8`, warm amber strings `#E4A548`, signal-soft class names `#F2C782`, body-color variables `#B9C5D2`. Light-mode counterparts `#2B6F9E` / `#A1671A` / `#263140` match the corresponding light-theme tokens exactly. **Side effect**: Pagefind index word count dropped 28822 → 26944 (−1,878 words, ~6.5%) because Shiki wraps each token in its own `<span>`. D20 row in `docs/DEBT.md` closes with this PR. **D21** Pagefind on Vercel Preview is the next polish residue item: blocked on Vercel Auth dashboard bypass for `/pagefind/*` + `/opengraph-image` + article route URLs (user-action, no code). **D42** destructive merge still pending user authorization. **D38 informational `verify-links` fail** (44 unresolved `related` refs in nextjs + nestjs submodules) is pre-existing and unchanged.
...
## Polish residue (Session 162, 2026-09-03)

**Session 161 D22 close + session 162 D20 close.** Two polish items shipped in one chain this session per user directive in session 161.

**Session 161:** `polish/d22-static-og` off `develop @ 2e52548`, PR #152 squash `9bbd2f1` — `/opengraph-image` route + `og:image`/`twitter:card` metadata on all 5 surfaces. Single static design per user decision ("site has no per-article art and social sharing volume is near zero"). D22 row in `docs/DEBT.md` closed.

**Session 162:** `polish/d20-shiki-buildtime` off `develop @ be9d25e`, PR #153 squash `46b77e6` — build-time Shiki syntax highlighting via `rehype-pretty-code@^0.14.5` + `shiki@^4.4.3`. Two custom themes at `apps/web/lib/shiki-theme-{dark,light}.json` hand-map every token scope to existing `@theme` tokens in `packages/ui/src/tokens.css` — NOT stock GitHub Dark / Light, per user directive ("dual-theme setup keyed to your existing tokens rather than a stock theme"). Dual-theme via `--shiki-dark` / `--shiki-light` CSS custom properties. `onVisitLine` callback + CSS counter produces the line-number gutter with zero JS. D20 row in `docs/DEBT.md` closed.

**Remaining polish residue:**
- **D21 Pagefind on Vercel Preview** — blocked on Vercel Auth dashboard bypass for `/pagefind/*` + `/opengraph-image` + article route URLs. Code shipped (PR #107); search works on localhost + `nxhhuy.tech` Production without any dashboard config. Preview deployments 302 to `vercel.com/sso-api` until bypass added.
- **D42 (items 1-4)** — code exists on `recovered-d42-merge` (HEAD `01e4aa4`), not shipped to `develop`. PR #151 on GitHub is **Closed** (not Merged). Destructive op (`git merge --no-ff recovered-d42-merge`) requires user authorization.
