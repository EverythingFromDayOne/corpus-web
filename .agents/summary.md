...
>
| **Last updated: 2026-09-04 (fix/verify-links-roadmap-classification — D13 closed → D46 opened on `develop @ 4851dc3`, merged as PR #157). 5 files modified +2 new: `scripts/lib/roadmap-manifest.mjs` NEW (~150-line per-submodule roadmap parser, `BACKTICKED_SLUG` regex + `SECTION_HEADING` regex + `MANIFEST_BOUNDS` per-repo size range + `assertManifestSizes` bounds check); `scripts/lib/adapt-all.mjs` exposes `manifestsByRepo`; `scripts/lib/link-report.mjs` accepts `manifestsByRepo` and promotes manifest-hit refs from `unresolvedTargets` (FAIL) to `plannedTargets` (WARN); `scripts/verify-links.mjs` + `scripts/build-catalog.mjs` plumb manifests through and emit `roadmap manifest: nextjs=N, nestjs=N, react=N, angular=0` once per run; `packages/content-schema/test/link-report.test.ts` NEW (9 tests pinning the classification boundary with fixture manifests). **Result against `develop @ 4851dc3`:** FAIL collapses from 44 → 19 (D13 group 4 only — 19 nestjs recipe refs with no manifest entry); WARN planned rises from 0 → 25 (D13 groups 1+2+3); WARN demo unchanged at 6. **Two parser bugs caught by the bounds-check during verification:** (1) `### Wave 1 — the spine` was matching `^##\s+\S+` — fixed to `^##\s+[^#]`; (2) BACKTICKED_PATH required at least one slash, missing all single-segment basenames like `providers-and-di` — fixed to BACKTICKED_SLUG with zero-or-more segments. **Test count:** 26 → 35 (+9 link-report tests). **DEBT.md:** D13 closed (groups 1+2+3 promoted to WARN via roadmap-classification); D46 opened for the 19 surviving FAIL refs. **No submodule changes.** All other cheap gates green: typecheck ✓, lint ✓, test 35/35, agents:check ✓, verify:prerender 196/196+18/18, verify:frontmatter 196/196. Exit code 1 — gate still fails on D46 (correct, D46 is the real defect this PR surfaces). PR #157 merged 2026-09-04 (merge commit `4851dc3`).
...
- **D45 (`readFileSync` at request time in `article-source.ts:8`)** — opened 2026-09-04. Same emit-then-static-import pattern as D43 will close it; needs deployed verification. Closure is `fix/d43-article-source`, deferred until D43's own deployed verification closes (so we know the pattern actually breaks in prod before writing the second instance of the fix).
