...
>
| **Last updated: 2026-08-31 (Polish-search-fixes-v2 in flight — `polish/search-fixes-v2` cut off `develop`. Two regressions remained after polish/search-fixes merged: (a) "icons overlapping" — actually two keycap chips rendered at the same coordinate. The dialog had a `<form method="dialog">` with `<button class="srch-kbd">Esc</button>` positioned absolutely at top-right via `.srch-dialog-close { position: absolute; top: 0.6rem; right: 0.6rem; }`, AND a `<kbd class="srch-kbd">⌘K</kbd>` in the input row. Both used `.srch-kbd` styling → top-right corner rendered as overlapping boxes (user's red-circle callout). Fix is **removal**: native `<dialog>` already handles Esc via the platform; the explicit Esc button was redundant AND caused the overlap. `apps/web/components/chrome/search-dialog.tsx` deletes the `<form>` + button; `apps/web/app/globals.css` deletes `.srch-dialog-close` and `.srch-dialog-close button` rules; `apps/web/messages/en.json` removes orphaned `placeholders.searchCloseLabel`. (b) "search function still fail" — the diagnostic detail from the previous PR showed "Search index failed to load. The /pagefind/ bundle may be blocked or unreachable." The fix path was right (discriminated-union error message worked, evidenced in the user's screenshot) but the load detection itself was a 3s blind poll that couldn't distinguish three different failure modes. New logic: dynamic `<script>` gets `onload`/`onerror` listeners; Promise resolves on load, rejects on error or 15s timeout. Post-script-load poll for `window.pagefind` bumped from 3s (50×60ms) to 10s (100×100ms). Three distinct error messages now surface the actual cause: "Pagefind script failed to load (network error or 4xx/5xx)" / "Pagefind script timed out after 15s" / "Pagefind bundle loaded but did not register window.pagefind within 10s. The runtime may be incompatible." 3 files changed, +28 / −33. All 5 gates green: typecheck 5/5, lint 0 problems, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196, vitest 38/38 pass / 0 fail. HTML spot-check on `/en/blog.html`: no `<form method="dialog">`, no `.srch-dialog-close` element; only the platform-managed Esc behaviour remains. Brand-string + personal-content guards: 0 hits. **Next:** same residue (D20 Shiki, D30 FAQ, D22 OG image, develop→main release) — all need user green-light. **If search "Search failed" reappears after this PR, the new error detail line will surface the actual cause (network error / timeout / runtime init failure) instead of the generic bundle-blocked message — paste it back to chat and the diagnosis is one round-trip away.** |
---
