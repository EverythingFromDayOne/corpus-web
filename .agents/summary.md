...
>
| **Last updated: 2026-08-31 (Polish-6 merged — `polish/d21-pagefind` cut off `develop` (`efe88e8`). DEBT D21 closed: `pagefind 1.5.2` declared in `apps/web/devDependencies`; new `postbuild` + `search:index` scripts run `pagefind --site .next/server/app --output-path public/pagefind` and index 221 pages / 28822 words in ~2.3s. Native `<dialog>`-backed `<SearchDialog>` mounted at `[locale]/layout` level; `<SearchTrigger>` `<button>` replaces the disabled `SearchPlaceholder` at the topbar's right edge. Opens on ⌘K / Ctrl+K; debounced 80ms queries; up to 8 results with Pagefind excerpts; ArrowUp/Down + Enter navigation. Bracket `[param]` Cache Components placeholder shells are skipped because they have no `<html>` element — the same exclusion rule `verify:prerender` already enforces. `apps/web/components/chrome/search-placeholder.tsx` deleted. +9 keys in `apps/web/messages/en.json`; +152 CSS lines in `apps/web/app/globals.css`. Predecessor (PR #96) merged first via squash on 2026-08-31: hero bloom + `.blog-content` typography block + post-header template + second section-divider use site + 15 `--blog-*` tokens. Brand-string guard 0 hits; personal-content guard 0 hits. Polish-7 PR #98 (D22 SEO residue: sitemap + robots.txt) and Polish-8 PR #99 (D25 `/en/license` + footer) still MERGEABLE on develop behind PR #97. **Next:** Polish-9 candidates per Phase 1 residue (D20 Shiki ⚪, D29 category-filter wiring, D32 related-articles section with 289 edges already in catalog); OG image generator (D22 residue — needs DNS + Vercel routing green-light in its own session); develop→main release PR is reserved for user action.**)
---
