...
>
| **Last updated: 2026-09-04 (docs/d38-close — D38 closed (superseded + already-fixed) on `develop @ cabd33b`, NOT YET PUSHED). 1 debt row status-flipped (no new ID issued; D46 stays the highest): `docs/DEBT.md` — D38 row in Open rewritten to "closed 2026-09-04, superseded" sentinel form (matching D39/D40 pattern); new D38 row added in Closed section with closure metadata. D38(a)'s 44-ref verify-links count was superseded by D13's fix (PR #157, now D46: 19 FAIL / 15 distinct); D38(b)'s three `derive-title.test.ts` failures are no longer reproducing (re-verified 2026-09-04, `node --import tsx --test packages/content-schema/test/derive-title.test.ts` reports 11/11 pass, 0 fail). 
