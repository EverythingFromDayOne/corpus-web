...
>
| **Last updated: 2026-09-04 (Polish-d43-answer-keys — D43 partial close on `develop @ f41b5c4`. 4 files +1 new dir +1 new script: `apps/web/lib/{quiz,dragdrop}-actions.ts` rewrote to static-import `apps/web/lib/data/answer-keys.ts` (NEW 31,991-byte build artifact emitted by `scripts/build-answer-keys.mjs`, 6 articles + 27 quiz keys + 6 dragdrop keys); `apps/web/package.json` `prebuild` hook wired to run the new emit script before `next build`. All 6 cheap gates green: typecheck 5/5, lint 5/5, test 3/3, build clean, `pnpm verify:prerender` 196/196+18/18, `pnpm verify:frontmatter` 196/196, `pnpm agents:check` ✓, `hermes verify --json` ok=true 9/9 phases PASS readiness HTTP 200 in 8.95s. **The Quiz is STILL broken in production** — local probe of `pnpm start` shows 0 `$F<id>` action references in the rendered HTML across 114 inline scripts. Cache-boundary action-reference loss (`apps/web/lib/article-markdown.tsx:207` has `'use cache'` and the `<Quiz gradeAction={gradeQuizAnswerForClient}>` JSX binding at line 390 is inside the cached scope) is the gating dependency. **Two independent bugs**: D43 = ENOENT catalog.json (fixed this session), D44 = cache-boundary (still open; user told me to set aside the cache-boundary refactor in session 165, but the investigation found BOTH bugs exist independently). Not pushed to a Preview URL — per user strict directive "Verification is a blocker, not a checkbox." Stash was popped, refactor dropped, work-in-progress on `develop @ f41b5c4` not yet committed. Polish residue: D43 partial close (article-source.ts:8 same fs-read pattern still unfixed), D44 new (cache-boundary action-reference loss), D42 items 7-8 (different defect shape, deferred per session 158), D38 informational `verify-links` (44 unresolved refs, pre-existing).**
...
- **D44 (cache-boundary action-reference loss)** — opened 2026-09-04 after `pnpm start` local probe revealed 0 `$F<id>` action references in rendered HTML. The `'use cache'` directive at `apps/web/lib/article-markdown.tsx:207` captures the `<Quiz gradeAction={gradeQuizAnswerForClient}>` JSX binding at line 390 — under Cache Components the action reference is dropped from the RSC payload. **Gating dependency for the Quiz to work in production.** The session-165 stash held the refactor (split `renderArticleMarkdown` into cached body renderer + non-cached wrapper); user told me to set it aside pending the real-fault investigation, but the investigation found TWO independent bugs (D43 + D44). Apply the stashed refactor once user authorizes.
