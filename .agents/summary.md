...
>
| **Last updated: 2026-08-30 (PR #90 opened: design-spec polish batch 2 — D20 items 3 (card hover accent), 4 (film-grain noise overlay), 5 (share buttons FB/X). Three commits on `polish/d20-batch-2` off `main` at `8378947`. Build + prerender green; 196/196 blog HTML + 18/18 lesson HTML. `verify-links` fails on the pre-existing 44 unresolved refs / 33 distinct targets (D13) — recorded in PR body, not chased. **New gotcha:** i18n keys for `ShareButtons` must be nested under `article.share.*` to match the existing `article.sectionDividerLabel` pattern; build prerender throws `Missing message: share.label` for top-level `share.*` paths. Caught by the build, not by reading the i18n file ahead of the keys — the same prerender-first rule would have caught the prompt too. **Submodule drift:** `verify-submodules` reports `react@v0.6.0` / `angular@v0.3.2` while `progress.md` still says `react@v0.5.0` / `angular@v0.3.0`; actual adapting count is 196/196, not 181. Doc state refresh is a separate session.**)** |
