...
>
| **Last updated: 2026-08-31 (Polish-11 in flight — `polish/d32-related-articles-polish` cut off `develop`. DEBT D32 closed: `apps/web/components/article/article-view.tsx` `RelatedList` now renders unresolved `related` refs (D13) with a distinct "unavailable" affordance — `◌` glyph prefix + `text-muted italic` styling + `aria-label="<slug> — related, not yet available"` for assistive tech + a hover `title` tooltip explaining the gap. The section already rendered (it was there from earlier work); what was missing was the visual distinction between working `<a>` links and unresolved spans. `apps/web/messages/en.json` adds `article.relatedUnresolvedTitle` + `article.relatedUnresolvedBody` under the existing `article.*` namespace. Catalog measurement: 102 articles carry ≥1 unresolved edge (495 unresolved of 289 total related edges). HTML spot-check on `/en/blog/nextjs/cache-components-model`: 5 related → 1 `<a href>` + 4 `av-related-unresolved` `<li>`s, exactly matching the catalog's 1+4 split. 24 +/− 1 in `article-view.tsx`, +2 i18n keys. All 5 gates green: typecheck 5/5, lint 0 problems, next build 236/236 (no new routes), verify:prerender 196/196+18/18, verify:frontmatter 196/196. Brand-string + personal-content guards: 0 hits. Predecessor PRs: #96 Polish-5 batch-5, #97 Polish-6, #98 Polish-7, #99 Polish-8, #100 Polish-9 hotfix, #101 Polish-9 D29 partial, #102 Polish-10 D30 timeline partial. **Next:** Polish-12 candidates per Phase 1 residue — D20 Shiki code blocks (only remaining ⚪ Phase-1 row; new dep `shiki` ~50MB grammars — needs user green-light per session protocol), D30 FAQ accordion (schema field `Path.faqs` — needs corpus-side authoring), D22 OG image generator (needs DNS + Vercel routing green-light in its own session); develop→main release PR remains reserved for user action.**)
---
