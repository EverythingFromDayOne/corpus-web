...
>
| **Last updated: 2026-08-31 (Polish-search-spotlight-ux mobile follow-up — `polish/search-spotlight-ux` off `develop` @ `72239fe`, PR #108. **Two mobile regressions fixed at the code level**: (1) dialog centring + "jump up when results arrive" → bulletproofed `.srch-dialog[open]` with explicit `inset: auto` (defeats UA `dialog { inset: 0 }` which re-centres via `margin: auto` and reflows as height changes), `top: max(1rem, env(safe-area-inset-top, 0px))` (clears iOS notch), `transform: translate(-50%, 0)` (no Y translation — pinned to top regardless of height), `max-height: calc(100dvh - 2 * safe-area-top - safe-area-bottom)` (respects iOS Safari URL-bar collapse). Verified in served bundle `.next/static/chunks/1v9knuy2qpoi4.css`. (2) "no way to close on mobile" → `matchMedia('(hover: none)')` touch detection (with Safari < 14 `addListener` fallback) renders an explicit `<button>Done</button>` in the input-row slot when query is empty on touch devices — calls `dialog.close()`. Desktop unaffected — Esc + backdrop-click still work. 2 code files + 1 i18n file (+37/-15), all 5 gates green. PR #108 updated.)** "Search failed" detail on Vercel preview deployments is caused by Vercel's Deployment Protection (Vercel Authentication ON for Preview on `develop.nxhhuy.tech`). Pagefind's Web Worker and chunk fetches are 302'd to `vercel.com/sso-api?url=...` because the auth cookie isn't sent by the unprivileged Worker thread. The fix is a **Vercel dashboard config change**: Project → Settings → Deployment Protection → "Path-based bypass" → add `/pagefind/*` (and ideally `/api/*` if any other protected-path needs it). Cannot be s... [truncated]
---
