...
import { coursesPath, homePath } from '@/lib/routes';

type Props = {
  params: Promise<{ locale: string; course: string; slug: string }> | undefined;
};
...
export default async function LessonNotFound({ params }: Props) {
  ...
  const course = resolved?.course ?? 'course';
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-24 text-center">
      ...
      <nav className="mt-8 flex justify-center gap-3 text-sm">
        ...
        >
          {t(messages, 'notFound.browseCourses')}
        </Link>
        <Link
