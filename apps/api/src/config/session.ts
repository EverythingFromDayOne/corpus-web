...
export async function buildSessionMiddleware(): Promise<RequestHandler> {
  ...
  return session({
    ...
    store: new PgStore({
      ...
      // `corpus_session` with the canonical DDL; the operator
      // applies it via `pnpm --filter @corpus/api migration:run`
      // post-deploy — see D71 for why boot-time migration
      // (`MigrationOnBootstrap`) was rejected. The runtime no
      // longer mutates the schema.
