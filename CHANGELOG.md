...
### [2026-09-05] — fix/d45-article-source-bodies — D45 close (preventive, D43 shape)

**Added**
- `scripts/build-article-bodies.mjs` — NEW (~165 lines). Walks `catalog.json`, reads every adapting article from the four content submodules, strips the YAML frontmatter with the same regex `article-source.ts:9` uses (kept in lock-step), emits `apps/web/lib/data/article-bodies.ts` as a typed static TS module. Mirrors the D43 `build-answer-keys.mjs` shape exactly: sorted `articleUids` for deterministic output, narrow `as const` + wide `Record<string, string>` cast for consumer ergonomics.

**Changed**
- `apps/web/lib/article-source.ts` — `readArticleMarkdown` no longer reads from disk at request time. Dropped `readFileSync` + `node:fs` import; the function now consumes the static module via a Set-guarded lookup. Same `'use cache'` + `cacheLife('max')` wrap; the static-import indirection resolves at bundle time, not at call time, so build-time cache hits still hold.
- `apps/web/package.json` — `prebuild` chain now runs `build-article-bodies.mjs` after `build-answer-keys.mjs`. The new script reads `catalog.json` directly, so it must come after `build-slug-allowlist` (which also reads the catalog and validates its shape).

**Fixed**
- `apps/web/lib/article-source.ts:8` D45 closed: same `readFileSync` at request time that D43 caught for Quiz/DragDrop grading. Currently dormant (the function is only invoked from the prerender pass, which runs in a container where the `content/<repo>/` submodule gitlinks are readable), but the moment any Server Action wires `readArticleMarkdown` into its body, the same `ENOENT /var/task/content/...` fires that D43 caught for the catalog. The fix is preventive — no deployed Verification probe is possible today because no such Server Action exists. Closes D45 with a forward guarantee.

**Generated artifacts (gitignored)**
- `apps/web/lib/data/article-bodies.ts` — 3933 KB for 196 articles, generated by `scripts/build-article-bodies.mjs` at prebuild time. Same treatment as `catalog.json`: too large to track across content submodule bumps, regenerated from source. Recovery: `pnpm build:catalog && node scripts/build-article-bodies.mjs`.
