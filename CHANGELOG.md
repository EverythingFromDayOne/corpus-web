...
- **Session 213** (2026-09-19, Lead, **D67 contract-violation finding** on `docs/fix-50-api-nestjs-rule`: empirical root cause of `pnpm --filter @corpus/api test` Case 1 failure verified via isolated probes (`/tmp/loadenv_probe2.mjs` + `/tmp/replay_case1.mjs` + `/tmp/case1_hex.mjs`) and byte-level audit of `.env`. `process.loadEnvFile()` itself PRESERVES pre-existing process.env entries (probe confirms); the override happens downstream in `toDatabaseUrl(env)` line 192-194 — `loadDotEnv()` re-populates `process.env.DATABASE_URL` from `.env` (because `withEnv()` strips it BEFORE `loadEnv()` is called), `Schema.safeParse(process.env)` parses it, and `toDatabaseUrl` returns the file's URL verbatim via its `if (env.DATABASE_URL) return env.DATABASE_URL;` first branch. CI is green because the CI env has no `.env` file — exactly the CI-vs-local divergence that hides precedence bugs of this class. Phase C blast radius: if a `.env` file is mounted alongside a Docker container, the file overrides injected config silently. **Test preserved unchanged** per Huy (Cases 2/3/4 are immune because they set DATABASE_URL directly; Case 1's failure is the test doing its job). Four candidate-fix options logged in D67 row; recommended (b) + (d) — `withEnv()` sets `dotenvLoaded = true` after stripping, AND replace `process.loadEnvFile()` with a manual parser that strictly respects pre-existing values (matches `dotenv.ts:40-42` existing pattern). 4 files changed: `docs/DEBT.md` (highest ID D66 → D67, D67 row added with full narrative), `CHANGELOG.md` (this entry), `.agents/SESSION-LOG.md` (Session 213 entry appended), `.agents/summary.md` (`Last updated:` line bumped). **No VPS claims**. **No test rewrite**. **Push holds** pending Huy: PR #191 is open awaiting merge.
- **Session 214** (2026-09-19, Lead, **D67 fix-shape decision (option f) + D68 row opened**): Huy's shell-vs-file precedence experiment (`POSTGRES_PASSWORD=from-shell node -e "process.loadEnvFile('.env'); console.log(...)"`) proved `process.loadEnvFile` honors precedence — initial hypothesis (`loadEnvFile` overwrites) was wrong. Real mechanism: `loadEnvFile` preserves pre-existing keys but fills MISSING keys from the file; `withEnv` strips `DATABASE_URL` before `loadEnv` runs, `loadDotEnv` re-populates from `.env`, `toDatabaseUrl` returns DATABASE_URL form first. Huy picked option (f): split `loadEnv` (pure parse, no `process.env` mutation) from `loadDotEnv` (population only); callers wire them in order via new `loadAppEnv()` wrapper with `appEnvLoaded` guard in wrapper, not `loadEnv`. Conditions: `loadEnv` stays exported + pure; `appEnvLoaded` guard in wrapper only. Rejected (c) — do not invert URL-vs-component precedence. New D68 row documents `auth.controller.ts` per-request env resolution as a separate finding (DI migration, hot-path file read + zod parse, out of scope for D67).
- **Session 215** (2026-09-19, Lead, **Phase B CLOSED + D69/D70 opened + PR #191 merged**): OAuth round-trip GREEN through Cloudflare Tunnel -> cloudflared (systemd) -> PM2 fork on :3001 -> Postgres. `/me` returns seeded user `edogawaconanhuyx98@gmail.com`. D-7 trust-proxy empirically confirmed (Secure cookie drops without it -> 401 on /me; was unobservable pre-TLS). Reboot test passed: cold start, zero VPS commands, `/healthz/ready` returns HTTP/2 200. VPS state: 4 tables (corpus_session, migrations, scaffold_healthcheck, users); first real user row at 2026-09-19 18:01:59. Off-box backup pulled + verified. **Failure on the way: `42P01 undefined_table`** because volume was reset during session 210 and nothing since had run migrations. **D69** = `/healthz/ready` cannot distinguish "connected" from "has a schema" (silent-fallback family from session 210; assert `migrations` table exists and non-empty; CI gate + startup-time migration:run). **D70** = `session.ts:104` `createTableIfMissing: true` is undocumented exception to `synchronize: true` rule (write `CreateCorpusSession` migration, apply, turn flag off -- Huy's preference). Highest ID D68 -> D70. **PR #191 merged** on origin/develop; HEAD now `67d2f86` (was `8d25a0e`).
...
- Highest ID issued D70 → D72.

**Changed**
- `apps/api/src/healthz/` Slice B implementation plan: `SchemaHealthIndicator` + test retained, `MigrationOnBootstrap` + `DatabaseModule` + `DatabaseModule` import in `app.module.ts` all dropped.
- `apps/api/src/db/migrations/` Slice C implementation plan: DDL rewritten to match measured live VPS exactly; constraint via `DO` block; new test shape (`pg_dump` round-trip + idempotency-against-already-applied).

### 2026-09-20 — docs/fix-50-api-nestjs-rule — Two `## Never-violate` rules ratified verbatim + corrected slice dispatched to BE

**Added**
- `.cursor/rules/20-never-violate.mdc` — new `## Verification discipline` section with verbatim directive from Huy (2026-09-20): *"When a brief says to measure, a derivation from documentation or package source is not a substitute. A gap found during verify is a blocker, not a footnote."* Attribution to D72 + PR #193 / `MigrationOnBootstrap` incident.
- `.cursor/rules/00-session-protocol.mdc` — new entry under `## Invented decisions — mandatory disclosure` with verbatim directive from Huy (2026-09-20): *"Quote, don't reconstruct. When restating what someone said — a count, a list, a decision — quote it or cite where it came from. A plausible reconstruction that ships into bookkeeping becomes a fact nobody can trace."* Attribution to D72 + Session 217 fabrication incident.
- `AGENTS.md` — regenerated by `pnpm agents:build` (8 lines added; both new rules correctly inlined). No change to `CLAUDE.md` or `.cursor/rules/60-skills.mdc`.

**Changed**
- `docs/DEBT.md` — D72 Open row amended: D72 standalone (not folded into D63) per Huy ratification ("D63 is runtime coverage of start:dev, D72 is what a migration test must assert. Different surfaces close on different conditions, same logic that split D57 from D63"); verbatim text of both new rules + operational "prove idempotency against a real database, not against migration strings" rule recorded.
- `docs/DEBT.md` — D70 Open row amended: constraint guard decision recorded as `pg_constraint` lookup wrapped in `DO` block (NOT alternatives — lookup is the condition, `DO` is the wrapper), with `conrelid` table-scoping line ("constraint names are unique per table, not per schema, so a bare `conname` check could match a different table's constraint entirely").
...
- `pnpm agents:build` ✓ (AGENTS.md regenerated, both rules inlined)
- `pnpm agents:check` ✓

**Invented decisions**
- Rule placement (`20-never-violate.mdc` §Verification discipline vs `00-session-protocol.mdc` §Invented decisions) — Huy ratified verbatim on both wordings; Lead's reading recorded but not deciding.
- Test design: `pg_dump` round-trip + real-DB idempotency in one extended test file (`apps/api/test/db/create-corpus-session-migration.test.ts`) — single file because both cases share the migration bootstrap; Huy may split on review.

**Next**
- Corrected slice dispatched to coding-be (`<@U0C1M7QFGF6>`) in auto-pilot mode targeting `fix/d69-d70-corrected` off `origin/develop @ 60b1547`. Brief at `~/.hermes/handoffs/20260920_d69_d70_corrected_dispatch.md` (16 KB; updated this session with the constraint-guard decision + operational idempotency rule).
