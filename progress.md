...
- **Session 224** (2026-09-22, Lead, PR #204 install-git-hooks worktree-aware fix): promotes the `f46b740` stash to develop on `fix/install-git-hooks-worktree-aware @ e31ad58`. Pre-fix `scripts/install-git-hooks.mjs` crashed with `ENOTDIR: not a directory, mkdir '.git/hooks'` whenever `pnpm install`'s postinstall ran inside a worktree (where `.git` is a 90-byte file containing `gitdir: ...`, not a directory) — silently no-oping the pre-commit hook install and breaking `hermes verify`'s bootstrap phase in worktrees. Fix branches on `statSync(.git).isDirectory()`: directory case unchanged; file case parses `gitdir:` pointer and takes `dirname(dirname(...))` to land at the SHARED `.git/` (the grandparent of `.git/worktrees/<name>/`) so `mkdirSync(join(realGitDir, 'hooks'))` is correct because worktrees share `.git/hooks/`. 2 commits (`0bcb320` +35/-8 the fix; `e31ad58` +11/-0 the CHANGELOG entry). CI on `e31ad58` re-triggered, workflow `35746082288` 4/4 `pull_request` checks SUCCESS — `Lint, typecheck, build` job runs `pnpm install` with the postinstall, proving the fix works on Vercel's build machines, not just locally. PR #204 MERGEABLE, 2 changed files +46/-8. No `--force-with-lease` — fresh branch, fresh push. Pre-commit hook fired on both commits; submodule pinning check exit 0 (D78 standing warning on `content/nestjs` tags remains, unrelated). `pnpm --filter @corpus/api typecheck/lint` exit 0, `pnpm agents:check` ✓. Bookkeeping on same branch: CHANGELOG/SESSION-LOG/summary/progress all updated; `docs/DEBT.md` Highest ID stays D72 (fix CLOSES D78 worktree-isolation blocker, not a new debt row). PR body at `~/.hermes/handoffs/v0.2.0/pr-installhooks-body.md` (3451 bytes, never `/tmp`). Out of scope (carry): #201 rebase + harness patch (step 1f, the only `--force-with-lease` in the batch — flagging before running), no-direct-push rule PR (step 1e, last to create), integration branch `batch/v0.2.0-features` (step 2). D73 (NEVER list is prompt-only) queued for post-release).

- **Session 225** (2026-09-22, Lead, integration branch `batch/v0.2.0-features` and PR #201 push): (a) created integration worktree `/Users/huynguyen/Documents/Self/corpus-web-batch-v020/` off `develop @ ab62ced`; first commit `f80de59` lands D73/D74/D75 (harness coverage gaps and prerequisites for PR #201's RED/GREEN analysis) per Huy's "save as patch, restore primary, apply as FIRST commit" directive; (b) pushed PR #201 (`fix/ui-evidence-signedin-coverage @ d09fc0a`) to origin with `--force-with-lease=...:2829340af8582aa175393159fb555a22d58be918` (correct lease-point syntax — first attempt with wrong syntax `d09fc0a9...:d09fc0a9...` failed with "stale info" as designed); applied PR body per Huy's verbatim instructions — signedIn mode fulfills /me with synthetic user so verifies chrome once auth state is resolved, but does not exercise failure path where /me never resolves; meBlocked fails the request but only checks for Sign in control, does not assert auth state resolved to signed-out. (c) Started integration merge order: PR #202 docs/rule-blocked-action-is-answer @ 78e8a6e (clean merge → `5afeb1f`), PR #205 docs/no-direct-push-protected-branches @ cb02855 (resolved summary.md/progress.md Session 222 entry preservation → `32f84bb`). Conflict resolution principle (Huy 2026-09-23): develop's version wins for any file no PR deliberately modified; a PR's version wins only where that PR intentionally changed the file; conflict outside four bookkeeping files → stop and ask. Out of scope: PR #201 diff-vs-develop shows PR #203's four commits because #201 sits on #203's branch — review by commit range `git log --oneline origin/feat/header-drawer-shared-account-control..origin/fix/ui-evidence-signedin-coverage`, not by diff.
