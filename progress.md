...
- **D45 closed (preventive) — fix/d45-article-source-bodies (2026-09-05, on `fix/d45-article-source-bodies @ 2c16d66`, PR #162 opened against develop, NOT YET MERGED):** Closed D45 with the same emit-then-static-import pattern D43 used for Quiz/DragDrop grading. `apps/web/lib/article-source.ts` rewritten: `readArticleMarkdown` no longer reads from disk at request time — drops `readFileSync` + `node:fs` import, consumes the static `apps/web/lib/data/article-bodies.ts` module via a Set-guarded lookup (`ARTICLE_UID_SET.has(article.uid)` → `articleBodiesByUid[uid]`). New `scripts/build-article-bodies.mjs` (~165 lines) reads `catalog.json`, walks every adapting article from the four content submodules (`content/<repo>/<sourcePath>`), strips the YAML frontmatter with the same regex `article-source.ts:9` used (kept in lock-step via comment), emits the static TS module with sorted `articleUids` for deterministic output. Mirrors the D43 `build-answer-keys.mjs` shape exactly: narrow `as const` + wide `Record<string, string>` cast for consumer ergonomics. Wired into `apps/web/package.json` `prebuild` chain (`build-slug-allowlist` → `build-answer-keys` → `build-article-bodies`). Generated module is 3933 KB for 196 articles — too large to track across content submodule bumps, so `apps/web/lib/data/article-bodies.ts` is **gitignored** (same treatment as `catalog.json`); recovery: `pnpm build:catalog && node scripts/build-article-bodies.mjs`. **Why "preventive":** `readArticleMarkdown` is currently only invoked from the prerender pass (`generateStaticParams` drives `next build`), which runs in a container where the `content/<repo>/` submodule gitlinks are readable. No Server Action calls it today, so there is no deployed `ENOENT` to reproduce. The fix closes the latent D43-shaped bug for the article body read before any future Server-Action wiring (`gradeReadingTimeEstimate()`, highlight-this-passage, etc.) can fire it. Defensive throws on unknown uid / missing body in `readArticleMarkdown` surface any drift between `catalog.json` and the static module loudly. **9/9 hermes verify phases PASS cold** (bootstrap 2.2s, build 73.139s cold / 1.496s cached, typecheck 9.169s, test 5.097s, lint 7.666s, readiness HTTP 200 in 11.499s). Live `curl` probes against `pnpm dev`: `/en/blog/nextjs/cache-components-model` 200 (839 KB, h1 + multiple `cacheLife` mentions confirm static-import rendered the actual markdown), `/en/courses/react-render-cycle/lessons/jsx-and-rendering` 200 (580 KB, lesson route also reads via the same static module), `/en/blog/nestjs/dependency-injection` 404 (correct — that slug doesn't exist). 6 cheap gates green: typecheck 5/5, lint 5/5, test 67/67 + 35/35 (total 102/102 across packages), agents:check ✓, verify:prerender 196/196 + 18/18, verify:frontmatter 196/196. `verify:links` still fails on D46 (19 refs / 15 distinct — unchanged, the only standing Content gate red per ADR-0003). `verify:catalog` exits 0 (196 articles, 445 edges, 2 paths). Bundle inspection: three `apps/web/.next/server/chunks/ssr/*.js` chunks reference `readArticleMarkdown`, confirming the static-import path is bundled. **Deployed verification is structurally weaker than for D43:** no prod-surface bug exists today to reproduce; the fix is a forward guarantee. Documented in DEBT.md row (preventive fix). 4 files changed, 235 insertions, 5 deletions. PR #162 awaits review + merge into develop per ADR-0003 Option A.
