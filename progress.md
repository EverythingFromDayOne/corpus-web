...
- **Session 218** (2026-09-21, BE, BE-1 auth redirect and logout hardening on `fix/api-auth-redirect-and-logout` off `develop @ 1d424b4`): three latent bugs in `apps/api/src/modules/auth/auth.controller.ts` + `apps/api/src/config/session.ts` fixed in one PR. (1) `WEB_ORIGIN.split(',')[0]` for redirect targets replaced by `resolveReturnOrigin(sessionReturnTo ?? referer, WEB_ORIGIN)` — exact-match against trimmed allowlist (NEVER `startsWith`, would pass `https://nxhhuy.tech.evil.com` → open redirect); backward-compatible via Referer fallback so develop keeps working before FE-1 ships `?returnTo=`. (2) Cookie attributes consolidated into new `buildSessionCookieOptions(env: AppEnv): CookieOptions` exported from `session.ts` — single source of truth for NAME+DOMAIN+PATH+SECURE+SAMESITE+MAX_AGE consumed by both `buildSessionMiddleware` AND `clearCookie` (the prior `clearCookie({ path: '/' })` cleared a different cookie than `session()` set when `SESSION_COOKIE_DOMAIN` was non-empty — a real logout bug masked by `session.destroy()`). (3) `req.logout?.(...)` replaced with `passportReq.logout(...)` + explicit `typeof === 'function'` guard that throws synchronously — missing-Passport no longer hangs the await forever. (4) New `CaptureReturnToMiddleware` applied via `AuthModule.configure(consumer).forRoutes('auth/google')` — captures `?returnTo=` into `req.session.returnTo` BEFORE `AuthGuard('google')` short-circuits, surviving the OAuth round-trip via Postgres-backed session. Three test groups in `apps/api/test/auth/redirect-and-logout.test.ts` (10+2+2=14 cases including the look-alike `https://nxhhuy.tech.evil.com → default` rejection). `pnpm --filter @corpus/api test` → 32/32 PASS in 2669ms; `CreateCorpusSession1700000002000` Test #1 pg_dump round-trip ran in 278ms against local `corpus-api-db` (proof the gate is honest, not stubbed). All gates green: `pnpm agents:check` ✓, `pnpm verify:submodules` ✓ (D37 standing warning unchanged), `pnpm verify:frontmatter` 196/196 ✓, `pnpm verify:links` 445/0/25/6 ✓ (unchanged from Session 217), `pnpm verify:catalog` 196/445/2 ✓, `pnpm lint` 5/5 ✓, `pnpm typecheck` 5/5 ✓, `pnpm build` 3/3 ✓ (api/web/mdx-components). **Hard-rule compliance:** no `synchronize: true`; no hand-edit of `packages/api-client/` (Swagger `@ApiTags`/`@ApiOperation` unchanged); no `*.spec.ts` co-located (new test follows `apps/api/test/auth/` sibling pattern per Session 211 tsconfig split); no `'server'` quiz mode; no new npm dep; no Express-cast (controller still uses Express `Request`/`Response` honestly). **No VPS claims** in PR body, commit message, CHANGELOG, or SESSION-LOG. Bookkeeping on same branch per branch-coupled-bookkeeping rule: `CHANGELOG.md` Session 218 entry under `[Unreleased]` (Added/Changed bullets only); `.agents/SESSION-LOG.md` Session 218 entry appended; `.agents/summary.md` `Last updated:` rotated to Session 218 (with full context); this line. **No debt rows opened** for the three fixed bugs — opening D73/D74/D75 just to immediately mark Closed would add zero audit value; the CHANGELOG + SESSION-LOG entries carry the trail. Out of slice scope (carried, NOT touched): BE-2 readiness-pending-migrations on `fix/api-readiness-schema-pending` (Lead's `consolidated-brief-before-v0.2.0.md` defines the slice — `dataSource.showMigrations()` check, `reason: 'schema'` → `check: 'schema'` rename, `dev:api` script chains `migration:run` before `start:dev`, real-DB test); D63 supertest CI gate; D73 bot identity / no-bypass token (Lead's separate debt-only PR); `docs/release.md` migration-requirement section (Phase 1 PR); `/cut` skill authoring (Phase 1 PR).

- **Session 219** (2026-09-22, BE, PR #198 passport sequencing regression test on `fix/api-auth-redirect-and-logout`): 1 new commit on top of `9bfef1b`. **NEW `apps/api/test/auth/oauth-callback.test.ts`** (148 lines, 2 cases) drives the `googleCallback` handler end-to-end via direct `new AuthController(appConfig)` construction with a `req.login()` mock that mirrors `passport/lib/sessionmanager.js:28-37` (deletes `sessionRef.returnTo` then `cb(null)` — simulates `req.session.regenerate()`). Case 1 asserts `captured.target === 'https://develop.nxhhuy.tech/auth/google/callback'` when `req.session.returnTo = 'https://develop.nxhhuy.tech'`. Case 2 asserts the first-allowlist-origin fallback when both `session.returnTo` and Referer are undefined — proves the test setup is honest about which path each case exercises. **RED-then-GREEN demonstrated**: temporarily mutated `auth.controller.ts:124` to call `req.login()` BEFORE the `sessionReturnTo` read (then `cp /tmp/auth.controller.ts.orig` revert) — Case 1 fails with `AssertionError: expected redirect to develop origin (from session.returnTo), got: https://nxhhuy.tech/auth/google/callback` (11ms); Case 2 still PASSES (proves RED is from regression, not setup glitch). After revert both GREEN. Full suite: `pnpm --filter @corpus/api test` 7 suites / 34 tests / 0 failures / 3452ms (was 6/32 pre-Session-219; +1 suite, +2 tests, all new). `pnpm --filter @corpus/api typecheck` exit 0. `pnpm --filter @corpus/api lint` exit 0. `pnpm --filter @corpus/api build` exit 0, no `*.test.js` leakage in `dist/`. All gates green: `pnpm agents:check` ✓, `pnpm verify:submodules` ✓ (D37 standing warning unchanged), `pnpm verify:frontmatter` 196/196 ✓, `pnpm verify:links` 445/0/25/6 ✓, `pnpm verify:catalog` 196/445/2 ✓ (after `pnpm build:catalog`). Conventional commit `test(api): regression — passport regenerate must not poison returnTo read` on top of `9bfef1b`. Same-branch follow-on to PR #198. Bookkeeping on same branch per branch-coupled-bookkeeping rule: `CHANGELOG.md` Session 219 entry under `[Unreleased]`, `.agents/SESSION-LOG.md` Session 219 entry appended, `progress.md` this line, `.agents/summary.md` `Last updated:` rotation. `docs/DEBT.md` Highest ID stays D72 (latent ordering bug closed by the new test, not filed as debt). **Worktree isolation note**: this work happened on a separate `git worktree` at `/Users/huynguyen/Documents/Self/corpus-web-pr198` because the main working directory had parallel-agent branch-switch collisions (same pattern FE agent flagged in their branch-hygiene report) — `git worktree add` per Huy-approved process gate. Out of scope (carry): BE-2 readiness/migration work (4 files in `stash@{0}`+`stash@{1}`+untracked commit `bcd2bbc`, branch `fix/api-readiness-schema-pending` lost to parallel-agent checkout) — needs rebranch from `origin/develop @ 1d424b4` + restore; Huy's question 3 from the Lead dispatch (BE-2 push timing) — my lean is "start now on develop tip, stay parallel" because the file trees don't overlap (`apps/api/src/health/*` and `apps/api/src/scripts/dev-api.ts` for BE-2 vs `apps/api/src/modules/auth/*` and `apps/api/test/auth/*` for PR #198).
- **Session 223** (2026-09-22, Lead, PR #198 BE-1 refactor: extract `executeLogout(req)` for test reuse on `fix/api-auth-redirect-and-logout`): 2 files, +88/-54. `apps/api/src/modules/auth/auth.controller.ts` — top-level `export async function executeLogout(req: Request): Promise<void>` added at the end of the file; `@Get('logout')` handler now calls `await executeLogout(req)` after the origin + cookieOptions computation (those stay on the controller because they're Express-coupled). `apps/api/test/auth/redirect-and-logout.test.ts` — imports the real export, drives it; Case 3.1's inline `run()` Promise wrapper removed; Case 3.2's inline run() removed; NEW Case 3.3 ("propagates session.destroy errors when logout succeeds") added (16 lines). `pnpm --filter @corpus/api test` → 35/35 PASS in 2951ms (was 7/34 pre-Session-223; +1 test, the new destroy-error case). typecheck/lint exit 0. RED-then-GREEN NOT demonstrated — refactor is pure code-motion, existing Case 3.1 already proved the controller's guard fails-loud and the refactor preserves that guard. Huy batch-7 rule: "don't pre-write RED expectations, run it and report what comes out." — applied. Worktree-isolation pattern reused from Session 219 (same `corpus-web-be` worktree, no branch-switch collision occurred). Bookkeeping on same branch per branch-coupled rule: CHANGELOG.md Session 223 entry under [Unreleased], .agents/SESSION-LOG.md Session 223 appended, .agents/summary.md `Last updated:` rotated, this line. docs/DEBT.md Highest ID stays D72 (no new debt row). Out of scope (carried): install-git-hooks PR (step 1d), no-direct-push rule PR (step 1e), #201 rebase + harness patch (step 1f), integration branch `batch/v0.2.0-features` (step 2). D73 debt row (bot identity / no-bypass token) queued for post-release per Lead dispatch.

- **Session 225 amendment 2** (2026-09-23, Lead, integration branch `batch/v0.2.0-features` and PR #198 merge): continued from Session 225 above and amendment 1. (a) integration worktree `/Users/huynguyen/Documents/Self/corpus-web-batch-v020/` off `develop @ ab62ced`; first commit `f80de59` lands D73/D74/D75. (b) pushed PR #201 (`fix/ui-evidence-signedin-coverage @ d09fc0a`) to origin with `--force-with-lease=...:2829340af8582aa175393159fb555a22d58be918`. (c) Started integration merge order: PR #202 docs/rule-blocked-action-is-answer @ 78e8a6e (clean → `5afeb1f`), PR #205 docs/no-direct-push-protected-branches @ cb02855 (resolved summary.md Session 222 preservation → `32f84bb`), PR #204 install-git-hooks worktree-aware (resolved summary.md Session 225→224→222 chain → `5c569dd`), PR #200 fix/api-readiness-schema-pending (resolved summary.md Session 225→224→222→220→217 chain → `a14b990`), PR #198 fix/api-auth-redirect-and-logout (currently resolving — taking theirs on summary.md and progress.md, prepend Session 225→224→222→220→217 chain on summary.md).
