...
- **Session 214** (2026-09-19, Lead, **D67 fix-shape (option f, `loadAppEnv` wrapper) decided + D68 row opened** on `docs/fix-50-api-nestjs-rule` ahead of PR #191 merge. Huy's shell-vs-file probe disproved initial `loadEnvFile`-overwrites hypothesis -- real mechanism is `loadDotEnv` re-populating `DATABASE_URL` from `.env` after `withEnv` strips it, then `toDatabaseUrl` short-circuits via line 192-194. 6 production call sites confirmed (no surprises). D68 = `auth.controller.ts` per-request env resolution via direct `loadEnv()` (DI migration, out of scope for D67). Push holds pending PR #191 merge; D67 implementation deferred to post-merge as fresh `fix/d67-loadenv-contract-split` branch.)
- **Session 215** (2026-09-19, Lead, **Phase B CLOSED + D69/D70 opened + PR #191 merged**): OAuth round-trip GREEN end-to-end (Tunnel -> cloudflared systemd -> PM2 fork :3001 -> Postgres); `/me` returns real user `edogawaconanhuyx98@gmail.com`. Reboot test passed (cold start, `/healthz/ready` HTTP/2 200, zero VPS commands). D-7 trust-proxy empirically confirmed. **D69** = readiness probe cannot distinguish "connected" from "has schema" (assert `migrations` table exists + non-empty). **D70** = `session.ts:104` `createTableIfMissing: true` undocumented exception (write `CreateCorpusSession` migration, turn flag off -- Huy's preference). Highest ID D68 -> D70. `origin/develop` now `67d2f86` (PR #191 merged). D67 branch `fix/d67-loadenv-contract-split` ready to cut next dispatch.
- **Session 217** (2026-09-20, Lead, response to Huy's BLOCKING directive on PR #193): PR #193 closed without merge (`fix/d68-d69-d70-config-hardening` deleted). Huy's `pg_dump --schema-only --table=corpus_session` against the live VPS revealed three deltas against the connect-pg-simple-package-inferred DDL shipped in PR #193: constraint name `session_pkey` (not `corpus_session_pkey`), index name `IDX_session_expire` (not `IDX_corpus_session_expire`), column types `character varying`/`json`/`timestamp(6)`. Huy also rejected the `MigrationOnBootstrap` boot-time hook citing four uncosted consequences (DDL on every PM2 restart, PM2 retry loops under `autorestart: true`, multi-instance race, weakening of D69 itself); verbatim close: "If you want boot-time migration, argue it as its own decision." Corrected slice dispatched to coding-be in auto-pilot mode targeting `fix/d69-d70-corrected` off `origin/develop @ 60b1547`: Slice A (D68) verbatim from PR #193, Slice B reduced to probe + test (drop `MigrationOnBootstrap` + `DatabaseModule` + the `DatabaseModule` import in `app.module.ts`), Slice C rewritten with measured DDL exactly (`DO` block for the constraint since `ADD CONSTRAINT` has no `IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS` for the index, exact column types) plus a new test shape (pg_dump round-trip diff + idempotency-against-already-applied). Huy applies the migration manually on VPS post-merge. `docs/DEBT.md`: Highest ID D70 → D72; D69 Open row amended with boot-time rejection paragraph; D70 Open row amended with schema-source correction paragraph; D71 Open row added (rejected `MigrationOnBootstrap` with four-objection rationale); D72 Open row added (rejected self-asserting DDL-string tests, points at the follow-up rule PR). New `## Never-violate` rule on "measure, don't derive" (Huy's directive verbatim: "when a brief says to measure, a derivation from documentation or package source is not a substitute, and a gap found during verify is a blocker, not a footnote"; D72 follow-through) NOT landed in this session — dispatched as a small independent PR with Huy sign-off on the wording. **Session 217 amendment (2026-09-20, same day):** Huy's directive enumerated four consequences (DDL-on-restart, PM2 retry loops, multi-instance race, D69 weakening), not five. Lead's initial D71/D69 row + Session 217 SESSION-LOG entry + Slack receipt had invented a fifth ("hidden failure mode far from the migration directory") not present in Huy's message — same class of error as the D70 "canonical by construction" derivation. All four files amended: "five uncosted consequences" → "four"; the fabricated fifth bullet removed.
- **Session 218** (2026-09-20, Lead, **two `## Never-violate` rules ratified verbatim + corrected slice dispatched to BE**): All three Session 217 decisions resolved by Huy this session — (1) **D72 standalone** (not folded into D63): "D63 is runtime coverage of start:dev, D72 is what a migration test must assert. Different surfaces close on different conditions, same logic that split D57 from D63." (2) **Two rules, not one**, with verbatim sign-off on both wordings — *"When a brief says to measure, a derivation from documentation or package source is not a substitute. A gap found during verify is a blocker, not a footnote."* → `20-never-violate.mdc` §Verification discipline; *"Quote, don't reconstruct. When restating what someone said — a count, a list, a decision — quote it or cite where it came from. A plausible reconstruction that ships into bookkeeping becomes a fact nobody can trace."* → `00-session-protocol.mdc` §Invented decisions. (3) **Constraint guard = `pg_constraint` lookup wrapped in `DO` block** — they are not alternatives, the lookup is the condition and the `DO` block is the only way to run conditional DDL in plain SQL. `conrelid` MUST be scoped to the table because constraint names are unique per table, not per schema, so a bare `conname` check could match something else entirely. Plus Huy's operational rule: **"prove the idempotency against a real database before claiming it, not against the strings the migration emits."** Files: `.cursor/rules/20-never-violate.mdc` §Verification discipline added, `.cursor/rules/00-session-protocol.mdc` §Invented decisions appended, AGENTS.md regenerated by `pnpm agents:build` (8-line add, both rules inlined), docs/DEBT.md D70 + D72 rows amended per decisions. Verify: `pnpm --filter web typecheck` ✓, `pnpm --filter api typecheck` ✓, `pnpm agents:build` ✓, `pnpm agents:check` ✓. Corrected slice dispatched to `<@U0C1M7QFGF6>` in auto-pilot mode targeting `fix/d69-d70-corrected` off `origin/develop @ 60b1547`; brief at `~/.hermes/handoffs/20260920_d69_d70_corrected_dispatch.md` (updated this session with constraint-guard decision + operational idempotency rule). Out of slice scope (carried): D63 `start:dev` CI gate, D64 (fold `25-react-provider-event-bus.mdc` into `20-never-violate.mdc`), D65 (Playwright E2E), D59 Huy Vercel re-verify, D8/D11/nightly backup. Post-merge: Huy applies migration on VPS manually.
- **Session 219** (2026-09-20, Lead, **PR #195 MERGED + VPS applied + D73/D74 opened**): Huy merged PR #195 (`d34f8cf` on `origin/develop`) and applied the migration on VPS. Receipts from Huy verbatim (in Slack thread): `pnpm --filter api migration:run` returned `migration:run — applied CreateCorpusSession1700000002000`; `pg_indexes WHERE tablename='corpus_session'` returned the expected 2 rows (`session_pkey` + `IDX_session_expire`) — DO block + IF NOT EXISTS no-opped correctly, no duplicate index, no PK conflict; `select name from migrations` returned the expected 3 rows in order; `curl localhost:3001/healthz/ready` returned `{"status":"ok","info":{"database":{"status":"up","reason":"schema","applied":3}},…}`; `curl -si localhost:3001/me` returned `HTTP/1.1 401 Unauthorized`. D69's connected-vs-has-schema gap is closed. **Two new debt rows opened** (Huy's verbatim directions): **D73** — Huy's verbatim ask from the prior turn ("test docstring says 'byte-for-byte against measured VPS DDL' but it runs against the local `corpus-api-db` container... fix the wording in the test docstring and in D72's row") was bundled into the comment-fix brief to BE, but PR #195 was merged after BE's `c66b938` (session.ts comment-only follow-up) without the test file or D72 row being touched. Direct read at `c66b938` confirms all four sites unchanged: test line 103 `it()` title, test line 112 internal comment, test line 149 failure message, `docs/DEBT.md` D72 row "Replacement test" paragraph. Additionally BE's `c66b938` introduced the same overreach in a new site: `apps/api/src/config/session.ts:102-110` now reads `…creates corpus_session with the canonical DDL matching the live VPS schema (measured via pg_dump --schema-only --table=corpus_session)` — same class of assertion, installed in runtime comment. D73 row catches both sites. **D74** — Huy's verbatim receipt ("`reason` reads as a failure field, so it looks odd next to `status: up`. Rename it when something else touches that indicator"): the `reason` field on a `status: up` indicator is a Terminus-shaped failure field; logging only per Huy's explicit "do not open a PR alone" direction. **Bookkeeping in this session**: `docs/DEBT.md` Highest ID bumped D72 → D74 with D73 and D74 rows appended verbatim from Huy's messages; `CHANGELOG.md` got TWO entries at top of `[Unreleased]` — the PR #195 merge entry (Session 219) AND a retroactive Session 218 entry (caught during this session's re-audit: `bec5665` committed Session 218 bookkeeping but missed CHANGELOG despite the commit message claiming "Session 218 bookkeeping," disclosed inline in the new CHANGELOG entry); `progress.md` this line. Stale follow-up brief `~/.hermes/handoffs/20260920_pr195_followup2_overreach_plus_naming.md` flagged in D73 row as superseded (the branch it targeted, `fix/d69-d70-corrected`, is now merged); kept on disk as historical record. Out of session scope (carried): D63 `start:dev` CI gate, D64/D65 (coding-fe scope), D59 Huy Vercel re-verify, D8/D11/nightly backup, D-5 second Postgres rotation, D-7 unobservable until TLS in front, header work (Huy to brief next).
