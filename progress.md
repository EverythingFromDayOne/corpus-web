...
- **Session 216** (2026-09-19, BE, D68/D69/D70 config hardening, branch `fix/d68-d69-d70-config-hardening` off `develop @ 60b1547` post-PR-#192 merge): three coordinated slices on one branch per Lead's dispatch brief. **Slice A (D68)**: new `apps/api/src/config/app-config.provider.ts` exports `APP_CONFIG` symbol + `appConfigProvider` (`FactoryProvider`, `useFactory: async () => await loadEnv()`); `auth.module.ts` registers the provider and re-exports the token; `auth.controller.ts` constructor gains `@Inject(APP_CONFIG) private readonly appConfig: AppEnv`, `loadAppEnv` import removed, both call sites (lines 86, 148) switch to `this.appConfig.WEB_ORIGIN` / `this.appConfig.SESSION_COOKIE_NAME`. `@Inject(...)` is EXPLICIT (same `D26 A.3` rationale — tsx/esbuild dev-mode loader does not emit `design:paramtypes`). Test: `apps/api/test/config/app-config.test.ts` (new, 4 cases) — factory idempotency (N=100 invocations return deepEqual snapshots), `APP_CONFIG` is a unique Symbol collision guard, factory returns `Promise<AppEnv>`, `provide` field equals `APP_CONFIG`. **Slice B (D69)**: new `apps/api/src/health/schema-health.indicator.ts` (`SchemaHealthIndicator` extends Terminus `HealthIndicator`, runs `SELECT to_regclass('public.migrations') IS NOT NULL` + `SELECT count(*)::text FROM migrations` in parallel, returns 503 `schema-missing` / `schema-empty` / 200 `schema { applied: N }`); new `apps/api/src/db/migration-on-bootstrap.ts` (`MigrationOnBootstrap implements OnApplicationBootstrap`, calls `dataSource.runMigrations({ transaction: 'each' })`); new `apps/api/src/db/database.module.ts` (one-liner `@Module({ providers: [MigrationOnBootstrap] })`); wired from `app.module.ts` after `TypeOrmModule.forRootAsync`; `ReadyController` constructor gains `@Inject(SchemaHealthIndicator)`, `check()` runs `pingCheck` AND `schema.check` in parallel; `HealthModule.providers` and `HealthModuleProviders` re-exports include the indicator. Test: `apps/api/test/health/schema-health.test.ts` (new, 5 cases) — 200 OK with applied count, 503 schema-missing, 503 schema-empty from count=0, 503 schema-empty from count-query race-window catch, Postgres `t`/`f` boolean shape. **Slice C (D70)**: new `apps/api/src/db/migrations/1700000002000-CreateCorpusSession.ts` with `CREATE TABLE IF NOT EXISTS "corpus_session" (sid varchar, sess json, expire timestamp(6))` + `ALTER TABLE ADD CONSTRAINT "corpus_session_pkey" PRIMARY KEY (sid)` + `CREATE INDEX IF NOT EXISTS "IDX_corpus_session_expire"` — DDL source is `connect-pg-simple@10.0.0/node_modules/connect-pg-simple/table.sql` (the package created the live VPS table via `createTableIfMissing: true` at first boot, package DDL is canonical by construction); `down()` drops index then table; `session.ts:104` `createTableIfMissing: true → false`. Test: `apps/api/test/db/create-corpus-session-migration.test.ts` (new, 4 cases) — canonical DDL strings, second-invocation idempotency, `down()` index-before-table ordering, stable `name` string. Tests: `pnpm --filter @corpus/api test` 18/18 PASS in 2887ms (4 D68 + 4 D69 + 4 D70 + 5 D57 + 1 D67). Typecheck/lint/build clean. `dist/` shape verified, new files compiled correctly, zero `*.test.js` leakage. Pre-commit gates: `pnpm agents:check` ✓, `pnpm verify:frontmatter` 196/196 ✓ (D37 standing submodule-tag warning unchanged), `pnpm verify:catalog` 196/445/2 valid ✓. **Four invented decisions** flagged for Huy ratification per dispatch brief: (1) D68 APP_CONFIG shape — full env snapshot via `loadEnv()`, not a typed subset (avoids future refactor if a third consumer reads a different value); (2) D69 lifecycle placement — `OnApplicationBootstrap` hook in a dedicated service, not inline `main.ts` between `app.init()` and `app.listen()` (NestJS-canonical); (3) D69 response shape — new `database: schema` field via Terminus's standard `HealthIndicatorResult` shape, not a separate top-level `migrations: <count>` field (preserves Terminus's per-indicator aggregation); (4) D70 schema source — `connect-pg-simple`'s `table.sql` (the package created the live table; package DDL is canonical by construction, no live-DB drift observed since boot). Hard-rule compliance: no `synchronize: true`, no new npm dep (all new types use already-installed `@nestjs/common`/`@nestjs/terminus`/`@nestjs/typeorm`), no `*.spec.ts` co-located, no hand-edit of `packages/api-client/` (no Swagger decorator changed on `auth.controller.ts`; `ReadyController` Terminus-owned response shape, no client regen required), no hard-delete of `lessons`, no `'server'` quiz mode, no canonical-NestJS-cast (no Express changes needed), no `SameSite=None`, no AGENTS.md hand-edit. Stop-and-ask triggers invoked: none hit. Bookkeeping: `docs/DEBT.md` Highest ID D67 → D70 + D68/D69/D70 Open rows (full narratives); `CHANGELOG.md` Session 216 entry; `.agents/SESSION-LOG.md` Session 216 entry appended; `progress.md` this line; `.agents/summary.md` lead-in rotation pending. Out of scope: D63 `start:dev` CI gate, D64/D65 (coding-fe), D59 (Huy Vercel re-verify). Auto-pilot mode per Huy's "review tomorrow".
