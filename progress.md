...
- **Session 225** (2026-09-22, Lead, PR #205 no-direct-push protected-branches rule): branches `docs/no-direct-push-protected-branches` from `78e8a6e` (PR #202 head) on `94e76a2`. Adds a new "Protected branches — no direct push from agent context" section to `.cursor/rules/20-never-violate.mdc` between "Safety mechanisms" and "Agent docs". Rule: "NEVER push to `develop` or `main` from an agent context. Bypass credentials inherit the ruleset exception; a direct push is neither blocked nor CI-tested. All bookkeeping, rescue, and one-line fixes go through a PR — batch into the next one if overhead is the concern. This is a permanent rule — agent identity does not change the rule." 1 commit on `79ea368`. PR #205 OPEN, CONFLICTING (expected; resolves on integration branch). Branched from PR #202 so the two rule PRs sit on top of each other; both modify `.cursor/rules/20-never-violate.mdc` in adjacent sections. AGENTS.md regenerated; CLAUDE.md and 60-skills.mdc unchanged (rule is always-on, not skill-triggered). Standing decision: do not write to `.git/` internals by hand in a worktree — commit messages go in `~/.hermes/handoffs/` and use `git commit -F`. `pnpm install --frozen-lockfile --ignore-scripts` used as workaround for the worktree ENOTDIR bug (PR #204 fixes this).
