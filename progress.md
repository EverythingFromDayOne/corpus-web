...
| D6 | **`nestjs-concepts` article 16 (`dtos-and-class-validator`)** — headline claim invalidated by article 17, `validationpipe-in-depth` (`forbidUnknownValues: false` is forced by Nest). Correction not yet applied upstream. Selectable as of `nestjs@v0.3.1` (D12 closed). | A known-false claim is pinned in `v0.3.1` and would render under `NEXT_PUBLIC_SHOW_DRAFTS=1`. Object-shaped `status` still collapses to `draft`, so production with drafts unset still does not ship it. | Fix in the corpus repo, then re-tag |
